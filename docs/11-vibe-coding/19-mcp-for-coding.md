...
> 译注：编码场景选 MCP 服务器的小抄——issue/PR 协作选 GitHub/GitLab 官方服务器；设计对齐选 Figma；数据库用 DBHub 之类 + 只读账号；内部系统自建 stdio 服务器并放进项目级 `.mcp.json`（凭据走环境变量）。MCP 与 Function Calling 的取舍对比见模块 9；在 CI 中用 MCP 的完整链路见本模块第 22 篇。
