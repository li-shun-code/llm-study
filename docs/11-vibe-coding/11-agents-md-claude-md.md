...
> 译注：本站实践建议——个人项目从一份 20 行以内的 AGENTS.md 起步，只写"AI 猜不到且会反复出错"的内容；团队项目把 AGENTS.md 作为唯一事实源，CLAUDE.md 用 `@AGENTS.md` 导入。Skills（第 12 篇）承接"多步骤流程"，Rules（第 9 篇）承接"按路径/按需加载"，三层各司其职。
