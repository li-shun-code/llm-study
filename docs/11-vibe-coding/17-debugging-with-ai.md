...
3. **独立 Bug 用干净会话**。Filippo 明确说"错误常量的上下文对排查独立 Bug 弊大于利"——上下文污染是真实的（第 16 篇讲过上下文 rot），`/clear` 不是失败而是策略。
...
> 下一篇预告：调试是"让代码回到正轨"，重构是"让代码走向更好的形态"。第 18 篇讲重构 with AI——从万行小步重构到百万行语言迁移，看大规模改动时的上下文策略。
