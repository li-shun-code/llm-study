...
**编者按**：本篇是"进阶实战与经典文献"部分的第三篇：Geoffrey Huntley《Ralph Wiggum as a "software engineer"》（2025-07-14）的**全文翻译**——"Ralph 循环"这一自律编码智能体工作法的命名出处（本模块第 13、14 篇曾摘引过它，第 30 篇作者也多次回指它）。一个 Bash while 循环 + 一份 PROMPT.md，让便宜模型通宵自主迭代直到交付：单进程单体、每循环一件事、子代理扩上下文、测试当背压、fix_plan.md 之外的"写作业"文化，全部在这篇里。文末附录的两份 CURSED 构建提示词原文照录。编者内容仅限本按语与文末署名说明。
...
Ralph 需要一种"不往主上下文窗口里分配"的心态。相反，你应该做的是孵化子代理（subagents）。你的主上下文窗口应该扮演调度器（scheduler）的角色，调度其他子代理去做昂贵的分配型工作，比如总结你的测试套件是否通过。

**我梦见 AI 子代理；它们在我睡觉时对我耳语**
...
另一件要意识到的事是：**你可以控制子代理的并行度。**

`84 squee (claude subagents) chasing <T>`（原视频截图说明）

如果你扇出几百个子代理，然后让这些子代理去运行应用的构建和测试，你会得到糟糕的背压（back pressure）。所以上面那条指令是：验证只能用单个子代理，但 Ralph 可以随心所欲地用任意多的子代理去搜索文件系统和写文件。
...
> **来源**：本文主体翻译自 [Ralph Wiggum as a "software engineer"](https://ghuntley.com/ralph/)（作者 Geoffrey Huntley，2025-07-14），署名翻译转载（作者公开博客允许署名翻译转载，需署名并附原文链接）。正文全文翻译（含两份附录提示词原文照录），未删节知识章节。抓取说明：ghuntley.com 全站对 curl 有 bot 防护（首次请求返回空响应），携带浏览器 UA 重试后抓取成功（85KB HTML，本地提取正文 3.2 万字符）；本站前轮曾以 Web Reader 抓取该页作摘译来源。抓取于 2026-09-13，开头编者按与文内译注为本站编者补充并已标明。
