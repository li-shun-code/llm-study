...
> 译注：Spec 驱动与站内其他篇章的关系——Spec Kit 的 `/speckit-implement` 本质是《Claude Code 工作流与最佳实践》里"可验证的完成标准"的项目化；`/speckit-tasks` 对应《从 0 到 1 用 AI 做产品：未来属于能直接动手的人》里 0→1 流程的任务拆解；`/speckit-converge` 与《AI 结对与代码审查》共享同一个思想：**让另一个上下文对照规格检查实现**。
