...
但**它们与本模块的主题（AI 编程的规范与上下文）不是同一条线**：它们教的是"怎么产出 Office 文件"，不是"怎么写更好的软件"。四个技能正文合计约 1.5 万字，全部展开会把这篇淹掉，而且这四个目录是**源码可用（source-available）而非开源**（各目录 `LICENSE.txt`），大段转载并不合适。**本站的处理是：只保留上面的定位判断与"每个技能最该记住的一条坑"；需要真正生成 Office 文档时，直接读仓库里对应目录的 `SKILL.md`。**
