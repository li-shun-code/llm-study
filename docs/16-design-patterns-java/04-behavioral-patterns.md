...
行为型模式关注对象之间的职责分配与消息协作：算法可以替换、事件可以广播、请求可以排队、状态可以外化。仓库在行为型（Behavioral）分类下收录 41 个模式，是最大的一个官方分类。本篇精讲 12 个核心模式——strategy、observer、chain-of-responsibility、command、template-method、state、visitor、mediator、memento、interpreter、iterator、null-object，全部为仓库实际存在的行为型模式。各模式意图与适用场景译自该模式目录 README 原文，代码引自仓库真实实现并标注路径。
...
- **策略**：Java 里一个策略对象常常就是一个 `Function`/`Predicate`/函数式接口。只有一两个算法变体、或变体可以写成 Lambda 时，传函数比建 `Context + Strategy + ConcreteStrategy×N` 三个类更清楚；`Map<Kind, Strategy>` 与枚举上的抽象方法也是同一个模式的轻量落地。反过来，策略需要携带状态或多个协作方法时才值得对象化。
- **观察者**：三种情况不要用它——① 需要同步、有顺序、且任一处理器失败就要中止的流水线（异常在广播里被吞或被放大，责任链更合适）；② 需要"恰好一次"投递语义（进程重启会丢通知，改用持久化队列或事务性发件箱）；③ 订阅者生命周期不受控（注册了不注销 → 内存泄漏，这在 Android/前端尤其经典）。广播风暴的早期信号是"一个事件触发 6 个 handler，谁也说不清最终状态是什么"。
- **责任链**：当处理顺序**有契约意义**时，隐式的"看谁认领"会让排错变成考古。链上节点超过七八个、或需要"必须全部执行"（校验器集合）时，改成显式有序的列表循环。
- **命令**：不需要撤销、排队、日志重放、跨进程传输时，一次方法调用即可——命令对象带来的序列化/版本兼容成本是长期的（命令类改了字段，历史命令还能不能重放？）。事件溯源系统里"命令与事件都进日志"是正确用法，但也意味着事件 schema 要有版本策略。
- **模板方法**：钩子点一旦多到五个以上，说明你其实在做组合而不是继承——把可变步骤抽成 `Function` 传进来（Strategy 化）或用 default 方法。Java 里还有一条硬约束：模板方法要求子类服从父类的调用顺序，这对测试与调试都不友好。
- **状态**：状态数 ≤3 且转换关系简单时，枚举 + `switch` 表达式（Java 14+）比一堆状态类更直白；状态数上去、且转换需要声明式审计（谁能从 A 到 B）时，才上状态机库或图。别把状态模式与工作流图混为一谈：**当"状态"活在配置里而不是代码里时，图/DSL 更合适**。
- **访问者**：它把变化点从"操作"移到了"结构"——一旦节点类型经常增删，每个 visitor 都要改，代价立刻超过收益。Java 21 的 `switch` 模式匹配 + sealed 接口已经能覆盖大部分"对一组固定类型分派"的需求，写 visitor 前先问一句：sealed 的穷尽性检查够不够？双分派带来的反射式访问者（`accept(this)`）在跨模块场景还会撞上循环依赖。
- **中介者**：中介者极易长成上帝对象（所有判断都在这里，因为它知道所有人）。它适合"通信拓扑复杂但协作规则集中"；不适合"每个协作方都有独立业务规则"。
- **备忘录**：快照的内存成本要算——大对象的深拷贝在 LLM 服务里可能一次几 MB。频繁存档时改事件溯源/差分快照（只存与上次的差异）。
- **解释器**：文法复杂就交给解析器生成器或现成 DSL（JsonPath、CEL、SQL 表达式、`jq`），别手搓语法树；仓库自己的 README 也提示"文法简单 + 效率不敏感"才是该模式的前提。
- **迭代器**：Java 已有 `Iterator`/`Spliterator`/Stream，Python 有生成器；重复造迭代器最常见的坏处是**丢失惰性**（`List.copyOf` 藏在里面，一次拉爆内存）。
- **空对象**：当"没有值"与"值恰好是默认"必须可区分时，`Optional`/`null` 检查更诚实。空对象适合"确实有语义上的默认行为"（`NoOpLogger`、`NullPaymentGateway`）。

## 十四、在 LLM / Dify 类项目里的实证

（以下结论引自本站《Dify 工作流引擎源码解析——节点编排、变量系统与沙箱执行》《n8n 执行引擎源码解析——任务运行器与节点式自动化》）

- **观察者：引擎只吐事件，宿主决定怎么用**：Dify 把 `Engine.run()` 设计成**事件生成器**——工作线程异步执行节点、主循环同步吐事件，任何未捕获异常都收敛为一个 `GraphRunFailedEvent`，调用方永远拿到事件而不是崩溃。这是观察者最干净的落地：生产者对消费者数量与身份零假设，SSE 推送、数据库落库、指标统计各自订阅，互不知晓。它同时验证了上一条判据的反面——**这里不需要顺序保证，也不需要恰好一次**（持久化侧靠幂等与 run id 收敛），所以观察者是对的。
- **n8n 的 hooks/`@OnEvent` 订阅**：n8n 的审计、日志、外部 webhook 通知通过事件订阅挂载在主流程之外；出现"事件在事务提交前触发、下游读到未提交数据"这类问题时，正确解法不是加 `sleep`，而是换成事务性发件箱或提交后事件（Spring 的 `@TransactionalEventListener` 就是为此存在）。
- **责任链：中间件与执行层**：Dify 用 `ExecutionLimitsLayer` 这类"引擎层"把"最多执行多少步、最多跑多久"集中实现，避免每个节点重复检查。这正是责任链/装饰型中间件的正用——**顺序有意义、且横切关注点必须不可绕过**；同时它也提醒反面判据：这类链要在装配处把顺序写清楚，否则限流层被放错位置等于没有限流。
- **策略：模型路由与退避重试**：按任务类型/成本/上下文长度选模型、按错误类别决定重试或降级（429 走指数退避 + 抖动、5xx 换 provider、内容审核失败直接终止）都是策略集合。Java 侧注意：策略数量一多，`Map<Reason, BackoffStrategy>` 或枚举携带行为通常比建一堆策略类更省事（见上一条判据）。
- **命令 + 队列：可重放的执行**：n8n 的 `packages/cli/src/workflow-runner.ts` 走队列模式，把"执行一次工作流"变成可跨进程投递的消息，天然具备排队、重试与水平扩展——这就是命令模式"在不同时刻指定、排队、执行请求"的原始动机。做 Agent 任务系统时，把每次工具调用做成命令（含幂等键），比用观察者硬连调用链可恢复得多。
- **模板方法：Task Pipeline**：Dify 的 `generate_task_pipeline.py` 把"流式响应的处理阶段"固定为骨架、各步骤可替换，是模板方法在生成链路里的自然形态；但一旦某一步需要按业务分叉，就应把该步抽成注入的函数而不是新增一层子类。
## 小结
