...
创建型模式最容易被滥用，因为它们解决的问题（"对象怎么造"）在每个类上都存在，而真需要模式的情况少得多。逐条给判据：

- **单例**：当你其实是想"隐藏依赖"时不要用——把 `Singleton.getInstance()` 散落在业务代码里，等价于给每个类加了隐式参数，单元测试必须靠反射或 mock 静态方法才能替掉它，这是最常见的测试地狱来源。有 Spring/DI 容器时，"每 JVM 一个实例"交给容器的 bean 作用域（`singleton` 是默认作用域）管理，不要自己写 `volatile` 双检锁：容器还负责销毁、代理与配置注入。需要"全局唯一 + 有状态"（连接池、计数器）时再考虑单例，且优先做成无共享的可变对象由容器托管。
- **工厂方法**：产品类型只有一种、构造也不依赖运行时上下文时，直接 `new`/静态工厂方法更清楚（《Effective Java》第 1 条正是"用静态工厂方法代替构造器"）。工厂方法的收益来自"选择哪个实现"这件事会变；如果它五年不变，工厂只是一层无信息量的包装。
- **抽象工厂**：只有一个产品族、或各产品之间没有"必须配套"的约束时不要用——它会把一次 `new` 膨胀成 `Factory + AbstractProduct×N + ConcreteFactory`。Java 里另一个信号：如果你发现工厂接口的方法返回类型都是 `Object` 或需要强转，多半是模型选错了。
- **建造者**：参数少于 4 个、或参数彼此独立时，用 record / 命名构造器 / 带默认值的方法链更省。Java 项目里 Lombok `@Builder` 或 record + `with*` 已能覆盖绝大多数场景；手写 builder 时如果它只做"把字段一个个赋值"（没有分步校验、没有不变量约束），那这层 builder 是纯噪声。
- **原型**：当"克隆出来的东西必须是新身份"（领域聚合根、订单、会话）时，简单 `clone()`/`record` 复制会带上原 ID，导致仓储覆盖写与事件错乱——此时要写显式的 `copyAsNew()`：新 ID + 复制状态 + 发一个"已创建"事件。浅拷贝陷阱（集合字段被两对象共享）在任何语言都是原型的第一号 bug。
- **对象池**：现代分配器 + 分代 GC 下，短命小对象的创建成本已经很低，池化的收益要用基准测试证明，而不是默认假设。池真正划算的前提是"底层资源昂贵且数量有限"（数据库连接、TLS 会话、GPU 上下文、文件句柄）。另一个代价是**状态泄漏**：归还时忘记 `reset()`，上一个请求的租户 ID、缓存、事务上下文就会跟着跑到下一个请求里——这是多租户平台的经典事故形态。还要注意**虚拟线程改变了线程池的语义**（Oracle JDK 21 起）：阻塞任务可以按"一任务一线程"写，不必再从池里借线程；但池作为限流闸门的价值仍在（见《行为型模式》的限流与背压），且 `synchronized` 块内阻塞会造成载体线程 pinning，池化 CPU 密集任务的做法依然保留。
- **惰性加载**：对象很小、或"必用"时，惰性只增加判空分支与首次访问延迟（还会把延迟尖刺推到线上第一个请求）。并发下首次访问的竞态要靠 DCL 或 holder 惯用法处理；Hibernate 场景下 `LazyInitializationException`（会话关闭后访问代理）几乎等价于"惰性把错误从编译期推到了运行期"。

## 十、在 LLM / Dify 类项目里的实证

把上面的判据放到真实 LLM 项目里，正反例都很清楚（下文的源码结论引自本站《Dify 工作流引擎源码解析——节点编排、变量系统与沙箱执行》与《n8n 执行引擎源码解析——任务运行器与节点式自动化》两篇的走读）：

- **工厂 + 注册表是对的**：Dify 的 `core/workflow/node_factory.py` 承担"按节点类型 + 版本解析出对应类，并注入 Dify 运行时依赖"的职责。这里的条件很典型——**产品类型是开放集合**（LLM、代码、知识检索、agent、各类 trigger 持续新增）、**同一类型有多个版本实现**、并且**构造需要宿主上下文**。这三条同时成立时，工厂方法（配注册表）就是最省事的形状；引擎侧只声明 `NodeFactory` 协议、宿主负责实现，属于"抽象工厂级别"的分层，而不是过度设计。
- **抽象工厂在这里也成立**：LLM 网关类组件（如 LiteLLM 及各框架的 provider 层）常需要"一族配套产品"——client + credentials + streaming decoder + embedding 接口，不同厂商的这组实现必须成套替换。单产品时才该退回工厂方法。
- **不要给配置对象做单例**：模型凭据、超时、特性开关这类东西做成进程级可变单例，在多租户服务里会造成"上一个请求改了全局配置"的串扰。Dify 的做法是配置随应用上下文（Flask `app`/请求上下文）与数据库中的 provider 记录走；本站的通用建议是：**配置当值对象传递，可变状态挂容器作用域**。
- **对象池的边界案例**：Dify 代码节点执行器（`core/helper/code_executor/code_executor.py`）选择"HTTP 调用独立沙箱服务"，而不是在主进程里池化子进程或解释器实例——因为每租户的资源配额、网络管控、依赖版本无法用池统一保证。这条判断可以直接平移：池化只有在"资源可互换"时才成立，带租户属性/权限属性的资源（沙箱、GPU、代理账号）不该池化。
- **惰性加载的实证**：模型权重与 tokenizer 的加载必须惰性（进程启动时装全部模型不可行），但要配合预热（warmup）与并发首载的串行化，否则第一个请求要为几 GB 的加载买单，还会引发并发重复加载。
## 小结
