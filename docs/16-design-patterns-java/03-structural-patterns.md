...
- **适配器**：不要写"透传型适配器"——只是换个方法名、参数一比一转发、不做任何语义转换，那层包装只会让调用链变长、堆栈变难读。适配器的成立条件是**两个接口真的不兼容**（参数结构、返回语义、错误约定、生命周期其中至少一项不同）。同时避免"万能适配器"：一个类里塞进五六个第三方 SDK 的转换逻辑，那已经不是适配器而是业务层。
- **桥接**：只有**一个实现维度**时不要拆抽象与实现——桥接的价值来自"抽象侧与实现侧各自独立扩展"，若实现只有 vendor A 一种且不会增加，它只会让你多维护一层接口。判据：数一下 `×` 号，若实现类数量 × 抽象类数量小于 4，通常是过度设计。
- **组合**：当"部分"与"整体"的行为差异是业务本质（例如只有容器可被授予权限、只有叶子可被编辑）时，不要为了统一客户端而强行抹平——`Composite` 里那些抛 `UnsupportedOperationException` 的方法就是设计在报警。
- **装饰器**：装饰链超过三四层，调试与性能都会变得难堪（每层一次虚调用 + 一层堆栈）。Web 框架的中间件本质就是装饰器的制度化，出现"我不知道请求经过哪些装饰"的困惑时，改成显式的处理器链并打印装配顺序。另一个反例：用装饰器做**状态变更**（而非职责追加）通常意味着你真正需要的是策略或状态。
- **外观**：外观应当**薄且无状态**。如果你发现外观里开始写业务规则、缓存、重试策略，它就是"上帝服务"的雏形——这些能力应该回到子系统或独立组件。也别为"少打几个 import"给两个不相关子系统做同一个外观。
- **享元**：收益必须实测。现代 JVM 有压缩指针、逃逸分析与分代回收，"少创建对象"经常不换来吞吐；而共享状态一旦可变就是并发 bug。`String.intern()` 是最典型的误用点：无节制驻留会撑大元空间/堆内字符串表，反而更慢。
- **代理**：动态代理依赖反射，单次调用成本与可读性都要付账；Spring 的 AOP 代理有两个经典失效场景（同类内部自调用绕过代理、`final` 方法/私有方法无法被代理），出现"事务注解不生效"就是撞上它。代理不是免费的 `@Transactional`，理解它生成的子类/接口代理形状再上生产。
- **私有类数据**：Java 的正解是 `final` 字段 + 不返回可变内部对象（必要时返回拷贝或只读视图）、乃至不可变 record。该模式在仓库里更像是对"别把字段暴露成 `public`"的教学示例，而不是推荐做法。

## 十、在 LLM / Dify 类项目里的实证

（以下结论引自本站《Dify 工作流引擎源码解析——节点编排、变量系统与沙箱执行》《n8n 执行引擎源码解析——任务运行器与节点式自动化》）

- **代理/适配当安全边界**：Dify 的 `core/helper/ssrf_proxy.py` 把所有用户发起的外部 HTTP 请求强制经过一层代理做目标地址校验，防 SSRF。这是"代理模式当横切安全层"的教科书用法——用户配置的 URL 不可信，因此校验不能写进每个节点（那是重复实现），也不能靠约定（会被绕过），必须成为不可绕开的结构。判据：**需要"客户端无法绕过"的约束时，代理是合理选择**，这时它不属于过度设计。
- **外观 over 复杂协议**：`core/helper/code_executor/` 把"选语言、包模板、鉴权头、超时、取 stdout/stderr"的多步 HTTP 协议收成一个 `execute(code, language)` 调用，配合 `TemplateTransformer` 把用户代码包进带超时与输出捕获的模板。这是**外观 + 适配器**的正当使用：子系统确实复杂，且多数客户端只要一个入口。
- **n8n 的 task runner 是代理 + 适配器**：`packages/cli/src/task-runners/task-runner-process-js.ts` 把"在独立进程里跑用户 JS 代码"包装成主进程内的一个本地代表，隔离崩溃与依赖冲突；调用方看不出对端是本进程还是子进程——这正是代理的定义。
- **多模型供应商：适配器的正当战场**：LLM 应用里各家 API 的差异不止方法名（流式协议、参数名、错误码语义、超时与重试约定、token 计数），因此 provider 适配层几乎必然要写；但**不要提前给"只有一个实现"的接口预留抽象**——先写具体调用，出现第二个厂商时再抽接口（那时你已经知道接口该长什么样）。
- **装饰器在中间件里的形态**：鉴权、限流、日志、Prompt 注入检测、响应缓存这类关注点常用装饰器/中间件实现。什么时候它变成"不该用"？当顺序有语义（限流必须在鉴权之后、缓存必须在鉴权之外）却没有显式声明装配顺序时——装饰器链的隐式顺序是 LLM 服务里 429 与 401 混在一起的常见原因。
## 小结
