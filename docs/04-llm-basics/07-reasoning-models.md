...
**推理能力已经从"专门产品线"变成旗舰模型的内建档位：2024 年 o1 首创"思考后回答"，2025 年初 DeepSeek-R1 用可验证奖励的强化学习把这条路线开源复现，随后各家族集体把它变成同一模型内的快答/深思双档。**

具体的发布时间、模型 ID 与各家推理档位名称，全站统一由本模块《主流模型生态对比（2026-09）》维护，本篇不再重复列表；本篇只讲**开发者侧的行为差异与调用契约**——这些变化不随版本更迭而失效。
...
- **多轮成本**：多轮对话中每轮都要重发不断增长的历史；开启"跨轮保留思考"会在此基础上叠加早前的思考条目。官方建议配合 Prompt Caching 降低重复输入成本（详见《成本与 Token 优化：Prompt Caching 与 Batch API》）。

## 五、reasoning_effort：思考深度旋钮

`reasoning_effort` 控制模型回答前思考多少。不同模型支持的取值不同，包括 `none`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max`，默认值也因模型而异（译自官方文档）。各家族的具体档位命名与哪些模型支持哪些取值，同样归《主流模型生态对比（2026-09）》统一维护，本篇不重复。

先记住这个旋钮的位置：`temperature` / `top_p` 改变的是**从分布里怎么抽**，`reasoning_effort` 改变的是**先算多少步再开始抽**——两者不在同一层。推理模型不接受前者（见《采样参数：temperature 与 top_p》第四节），所以"把温度调到 0 治幻觉"这条路在这里直接换成"把思考档位调高"：
...
学习建议：推理模型不是"更强的聊天模型"，而是**用 token 换正确率的工具**。把它放在你的应用里最难的 5% 问题上（复杂调试、深度规划、高风险判断），其余交给普通档位——这条"分层调用"原则同样适用于《LLM 驱动的自主智能体（Agent）全景：规划、记忆与工具使用》里的 Agent 架构设计与《开源模型选型：Qwen、GLM、DeepSeek 官方模型卡对照（2026-09）》里的私有化选型。

回到本篇的主线：《采样参数：temperature 与 top_p》给你随机性上的旋钮，《幻觉：成因与缓解》告诉你哪些错误来自随机性、哪些不来自，《推理模型（o1/R1 类）》则把"不来自随机性的那部分"用算力换掉——三者合起来才是一套完整的"输出不对时怎么办"。
