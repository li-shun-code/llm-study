...
如果你还没掌握 Git 的暂存/提交/分支/合并，请先学习「计算机基础」的 Git 基础两篇——《Git 基础（暂存/提交/回退）》与《Git 分支/合并/PR 协作》——再回到本篇。AI 工作流没有引入新的 Git 概念，它只是把三件事的优先级提到了最高：**提交是存档点**（智能体跑偏时回退的最小成本手段）、**分支是实验隔离舱**（多个智能体互不踩脚）、**审查是唯一的质量闸门**（你不再是唯一的作者）。
...
> 译注：worktree 是《Git 分支/合并/PR 协作》中"分支隔离"思想的多目录形态——分支给历史画叉，worktree 给工作区画叉。AI 时代它的价值被放大：每个智能体一个舱位，合并前互不干扰。
...
4. **审查是唯一闸门**：AI 生成代码一律走 PR review——Writer/Reviewer 双会话（《Claude Code 工作流与最佳实践》）、`/code-review` 技能或 Copilot/Bugbot（《AI 编程工具全景对比（2026-09 版）》）、加上人眼终审。审查流程详见《AI 结对与代码审查》。
