...
如果你还没掌握 Git 的暂存/提交/分支/合并，请先学习计算机基础模块的两篇——《Git 基础：获取仓库、暂存、提交与回退》与《Git 分支、合并与远程协作》——再回到本篇。AI 工作流没有引入新的 Git 概念，它只是把三件事的优先级提到了最高：**提交是存档点**（智能体跑偏时回退的最小成本手段）、**分支是实验隔离舱**（多个智能体互不踩脚）、**审查是唯一的质量闸门**（你不再是唯一的作者）。
