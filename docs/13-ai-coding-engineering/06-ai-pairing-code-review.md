...
AI 时代写代码的常态是"人机结对"：智能体写实现，人负责方向与验收。这个模式最大的风险在于**作者自己审查自己**——无论是人审 AI 的产出，还是 AI 偏袒自己刚写的代码。业界因此收敛出三条互补做法（见《Claude Code 工作流与最佳实践》 Claude Code 最佳实践）：
...
本文翻译的 Claude Code Code Review 文档，正是上面第三条（无人值守后的强制审查）的工业化形态——多智能体分析整个代码库、自动审查 GitHub PR、把发现钉在具体代码行上。同一赛道的其他玩家：GitHub Copilot 的 Copilot Code Review（PR 页面点 "Request Copilot review"）、Cursor 的 Bugbot、Cline 的 GitHub PR Review 样例（横向对比见《AI 编程工具全景对比（2026-09 版）》）。
...
> 译注：把"审查指令"写进仓库（REVIEW.md）、把"审查成本"暴露给团队（分析面板）、把"审查结论"做成可机读输出（check run JSON）——这套设计让 AI 审查可以被工程化管理，而不是一个黑盒评分器。它对应着本站多篇实践文章反复出现的母题：**AI 能力的每一次跃升，都把瓶颈推回工程纪律**。

## 可复制的审查提示词与合并清单（本站编者整理）

> 官方文档讲的是"机器怎么审"；这一节讲"人怎么提问题、怎么收口"。

**① 本地先自审一遍（合并前 3 分钟）**

```bash
/code-review                       # 审本分支领先上游的提交 + 未提交改动
/code-review main...my-feature     # 审指定 ref 区间
/code-review --fix                 # 审完把可自动应用的发现落到工作区
/code-review ultra                 # 关键变更的云端多智能体终审
```

**② 让独立会话做对抗式审查（不共享写作过程的上下文）**

```text
你是独立审查者，看不到作者意图，只依据以下事实判断：
- 判据：<spec.md 或 issue 的验收标准原文>
- 范围：`git diff origin/main...HEAD`
请只做四件事：
1) 逐条核对验收标准，给出"满足/不满足/无法判定 + 证据（文件:行号）"；
2) 找行为缺陷：错误路径、并发与竞态、边界值、资源泄漏、越权与数据泄露；
3) 列出 diff 中"未被要求"的改动；
4) 给出最多 5 条最重要的问题，其余只计数不展开。
禁止：改代码、格式化建议、以及"看起来更优雅"的重写提议。
```
...
- [ ] 机器审查的 🔴 Important 全部已修或有明确不修理由；🟡 Nit 未被当成阻塞项。
- [ ] 验收标准逐条有证据，而不是"我测过了"。
- [ ] diff 里没有无关改动；`tests/`、lockfile、生成文件的改动都在预期内。
- [ ] 日志与错误信息里没有邮箱、用户 ID、请求体、密钥（`REVIEW.md` 的"always check"项）。
- [ ] 数据访问按调用方租户/权限范围限定；新对外接口有鉴权与集成测试。
- [ ] 我能在 60 秒内向另一个人解释这个 PR 做了什么、为什么这样做——不能解释的不合并。

## 延伸阅读

- 《Git in AI 工作流：commit 即存档、worktree 隔离与审查流》——审查之前的分支与提交纪律
- 《AI 代码的安全与质量陷阱：六大陷阱与检测清单》——审查时最该盯的六类缺陷与检测命令
- 《Claude Code 工作流与最佳实践》——Writer/Reviewer 双会话与对抗式审查的完整流程
- 《Headless 与 CI 中的 AI 编码：Headless、Agent SDK 与 Copilot 云端智能体》——把审查接进自己的 CI 基础设施

---

> **来源**：本文主体翻译自 [Code Review](https://code.claude.com/docs/en/code-review)（Claude Code 官方文档，2026-09 当前版），作者 Anthropic，许可署名翻译（官方文档，Copyright Anthropic PBC，仅作教学用途翻译并署名）；`/code-review` 一节同时参照官方文档中该命令的说明；开头三条业界做法为编者摘要（对应出处已在正文标明）；"可复制的审查提示词与合并清单""延伸阅读"为本站编者内容并已标明。原文页面图片未搬运。抓取于 2026-09-13。
