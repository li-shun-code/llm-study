---
title: 数字与字符串（f-string 与 UTF-8 编码）
source_url: https://liaoxuefeng.com/books/python/basic/string-encoding/index.html
author: 廖雪峰
license: © 廖雪峰（转载署名）
...
| 中 | x（超出范围） | 01001110 00101101 | 11100100 10111000 10101101 |
...
上述代码中，`{r}`被变量`r`的值替换，`{s:.2f}`被变量`s`的值替换，并且`:`后面的`.2f`指定了格式化参数（即保留两位小数），因此，`{s:.2f}`的替换结果是`19.62`。f-string 是 Python 3.6 引入、如今最为推荐的字符串格式化写法。
