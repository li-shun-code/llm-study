...
translated: true
versions: LangSmith 当前版（docs.langchain.com）；langsmith Python SDK 0.13.0
order: 21
group: 生产化：可观测、评测与安全
---
# 为什么 Tracing 是 Agent 生产排障的第一工具

《多智能体模式：Anthropic 如何构建多智能体研究系统》里有一段亲身经历：用户报告智能体"找不到明显的信息"，团队却看不出为什么——是搜索词太差、来源选得不好，还是工具调用失败？**加上完整的生产追踪（Tracing）之后，才能系统性地诊断失败并修复**。
...
```text
Thread（保留嵌套）                          Trajectory（摊平为一份消息序列）
turn1 ─ trace A                             human: 上月电费超了，能退吗？
        ├─ chain: triage                    ai:     → transfer_to_refund
        │    └─ llm: 分诊                   tool:   refund_policy: 30 天内可退
        ├─ tool: refund_policy              ai:     → 可以，需订单号
        └─ llm: 生成回答                    turn2 ─ human: 订单号 A-1024
turn2 ─ trace B                             ai:     A-1024 已提交退款
        └─ llm: 生成回答                    （看不到 run 的嵌套与耗时）
```
...
- `@traceable` 装饰器：追踪任意函数。可传 `run_type`（`llm` / `chain` / `tool` / `retriever` / `prompt` 等，默认 `chain`）、`name`、`tags`、`metadata`、`project_name`、`client`、`process_inputs` / `process_outputs`（**脱敏入口**：想在上报前抹掉 PII 就在这里改）、`enabled`、`exceptions_to_handle`；被装饰的函数还会多出 `add_metadata()` / `add_tags()`，供调用处补运行时元数据。
- `trace(...)` 上下文管理器（Python）：包裹特定代码块，配合 `@traceable` 划出"一次业务操作"的边界。
- `RunTree` API：底层、显式地构造 trace（`RunTree(name=..., run_type=..., parent_run=...)`），自己管父子关系与 `dotted_order`，适合在框架之外手写插桩。

`run_type` 传错值不会静默通过，SDK 会提示合法取值并怀疑你把类型当成了名字（`Unrecognized run_type: xxx. Must be one of ... Did you mean @traceable(name='xxx')?`）。
