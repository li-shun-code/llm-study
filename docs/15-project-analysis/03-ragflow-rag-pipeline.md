...
不必上整套 RAGFlow，下面几条抽出来就能用在任何文档解析或检索系统里。

1. **让"位置"成为一等字段，一路带到索引**。DeepDoc 三件套（OCR / 版面 / 表格结构）产出的是带 `page_number / x0 / x1 / top / bottom / layout_type` 的 boxes 流，chunk 与索引里保留 `position_int`，于是"点引用跳回 PDF 原文并高亮矩形"才可能实现。照搬形式：解析层输出结构体（不是 String），从切块、入库到检索结果全程不丢页码与 bbox；哪怕暂时不做高亮 UI，也不要先丢了坐标——补回来的成本等于重跑一遍全量解析。这是"能溯源"与"只能给文本"的分水岭。
2. **能判定的切分细节交给小模型，别写脆弱的阈值规则**。相邻两行该不该拼成一句，RAGFlow 用的是仓库里那个几十 KB 的 XGBoost 二分类模型（`updown_concat_xgb.model`），特征手工构造（行距、对齐、字号、标点结尾），而不是"行距 < N 就合并"。可搬的做法：把解析里那些"规则怎么调都会在某些文档上崩"的判断点单独抽成一个可训练的小分类器，样本直接从 badcase 里标（100～500 条就有效），线上跑毫秒级、可离线、无 LLM 成本。同时把"只在生成与疑难摘要里用 LLM"作为默认分工。
3. **分块类型用注册表，领域知识沉在模板里**。`FACTORY = {parser_id: chunker}` 一行注册一种文档类型，`qa` chunker 复用同一套 DeepDoc 底座，只是把切分换成"按问题项目符号版式规律切"（`qbullets_category()` 识别 Q 行的前缀与 x 坐标对齐），并为问题单独建向量字段（`question_tks`）+ 查询侧高权重（`query_fields` 里的 `question_tks^20`）。抄这三件事：（a）解析底座与切分策略解耦；（b）新文档类型是加一个类而不是改主干；（c）如果用户查询是"问题"，就把语料的"问题面"单独向量化——字段级加权比调 embedding 模型便宜得多。
4. **检索与引用的打分要显式、可解释、并带阶梯兜底**。`rerank_by_model()` 的核心是 `tkweight * tksim + vtweight * vtsim + rank_fea`，最终每条 chunk 带 `similarity / term_similarity / vector_similarity` 三个分量；引用回填（`insert_citations`）则先把答案按中英句读切成句级片段（``` 围栏内容用 `re.split(r"(```)", answer)` 排除在外），逐句与 chunk 算混合相似度（权重反转为 0.1/0.9，更信语义），阈值从 0.63 起步，若整篇一句都没挂上引用就按 0.8 倍逐轮放宽到 0.3。可搬的工程点：送 reranker 的是**原始自然文本**而非分词后的 token（词干化会让 cross-encoder 系统性打低分，逼你把 `similarity_threshold` 越调越松）；不同 rerank 提供商的分数先归一到 [0,1] 再加权（否则换模型等于换标尺）；排序用稳定排序（`kind="stable"`）保证翻页不重不漏；与其定一个魔法阈值，不如定义"至少挂上一条 + 放宽有下限"的收敛规则并把生效阈值记进日志；三个分量存进响应，"为什么命中"就能直接给用户看，也是离线调权重的唯一依据。另外务必按句而非按答案整体匹配——否则一句多事实时引用会全落到第一个 chunk 上。
5. **把成本与一致性的兜底写进管线，而不是写在文档里**。embedding 批次超时与限流、删除文档后清理派生 chunk（`_prune_deleted_chunks`）、标题加权向量、繁简转换与中文编号正则这些"中文一等公民"处理，正是 RAGFlow 在生产可用与 demo 之间的差额。照搬清单：外部 embedding 调用一律带批次上限 + 超时 + 幂等重试；删除是级联的（源、chunk、索引、缓存四处都要有清理路径，缺一个就会检索到幽灵片段）；预处理里的语言特定规则集中成一个可测的模块，别散在切块代码里。
