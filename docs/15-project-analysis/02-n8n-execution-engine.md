...
挑五条最容易直接迁移的做法，每条给落地形式与判据。

1. **让「一次执行」等于一个 JSON 对象**。n8n 的 `runData[节点名][runIndex]` + `nodeExecutionStack` + `waitingExecution` + `waitTill` 全是可序列化纯数据，于是暂停、恢复、从出错节点重跑、历史回放都只是「把这份数据换个地方再喂给引擎」。照搬做法：把引擎入口设计成 `execute(snapshot) -> snapshot'`，数据库存快照而不是日志行；重跑接口只需要「取快照 → 截断到出错节点 → 再执行」三步。判据很硬：如果重启进程后无法从中间状态继续，就说明还有隐式状态留在内存/栈里。Java 侧配合用不可变 record + Jackson 多态类型（记得注册子类型，别用 `Object`），并且给快照带 `schemaVersion`。
2. **顺序语义要版本化，不要「顺手改」**。n8n 把多输入节点的执行顺序做成 `executionOrder: v0 | v1` 工作流级开关，老工作流继续按 v0 跑，新工作流默认 v1。任何被用户依赖的行为改变（哪怕是修 bug）都该走这条路：新增字段而非改变默认值、把工作流创建时间或显式配置作为开关来源、两条分支都要有测试。对照反例是「改了排序规则然后所有历史流程输出顺序变了」——在自动化与批处理场景这是数据事故。
3. **给每条输出数据加一个「血缘指针」字段**。`INodeExecutionData.pairedItem` 记录这条输出来自哪些输入 item，`addPairedItemLineage` 在执行前统一补全，因此任何节点出错都能定位到「第几条数据」，前端才能做单条重放与调试。工程上这是低成本高回报的一行代码：输出结构里加 `sourceIndices: number[]`，由框架在节点执行后统一填，而不是要求每个节点自己记得填（后者一定会漏）。数据流水线、ETL、批量 LLM 处理都适用。
4. **隔离不可信代码时，优先关掉解释器能力而不是过滤危险 API**。任务运行器的第一道防线是启动旗标：`--disallow-code-generation-from-strings`（禁运行时 `eval` / `new Function`）与 `--disable-proto=delete`（切断原型链污染入口），第二道才是 `prototype-hardening.ts` 冻结内部对象原型方法。此外三条同样可搬：子进程主动向主进程「揽活」（offer 轮询，形成天然背压与限流）、连接需 `grantToken` 鉴权、节点取数据走 RPC 按需拉取（子进程拿不到 DB 连接串）。落到 JVM 上对应做法是 SecurityManager 的替代路径：独立进程/容器 + 关反射与动态字节码（`-XX:-EnableDynamicAgentLoading`、禁 `--add-opens`）、只授予最小权限的 token、把 IO 收敛成受控接口。核心判断标准是「用户代码可枚举的能力面有多小」，而不是「黑名单写得多全」。
5. **把「等待外部」建模成协议返回，而不是异常或新线程**。`EngineRequest{actions, metadata}` 让 Agent 工具调用、人工审批、等 Webhook 共用同一条「请求-恢复」路径，主循环不为任一具体场景写特例；配套的「执行前 / 执行后」钩子让持久化与进度上报订阅同一组事件。照搬形式：节点返回类型做成判别联合 `{kind:'data', items} | {kind:'request', actions, requestId}`，恢复时按 `requestId` 幂等匹配并把响应塞回栈。收益是外部世界怎么扩（新增审批、新增异步工具）主循环都不用改，而且「挂起中的执行」本身是可序列化状态——与第 1 条合起来，就得到一个能长期存放、随时唤醒的执行模型。
