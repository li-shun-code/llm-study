...
group: 名篇全文翻译
---
**编者按**：本篇是 Harper Reed《My LLM codegen workflow atm》（2025-02-16 发布）的**全文翻译**——AI 代码生成工作流方向被引用最多的名篇之一：从"头脑风暴出规格、规格出计划、计划出提示词序列"的绿地流程，到基于 repomix/mise 的存量代码增量改造流程，每一处提示词原文照录。本站讲工具与机制的篇章回答"某个能力怎么用"，这一篇回答的是"一个完整的个人工作流长什么样"。编者内容仅限本按语、文中已标明的译注，以及文末"2026 年回看"一节。
...
> **本节为编者注，非原文内容。** 原文写于 2025 年 2 月，是 AI 代码生成工作流方向被引用最多的文章之一。下面按"已成共识 / 已被工具内置 / 今天怎么用"三档给出回看判断，全部是编者观点。

**已成共识（当年这篇属于先行表达）**

- **"先出规格、再出计划、最后才出代码"的三段式**成了主流工作流骨架：今天 Claude Code 的计划模式、`/specify → /plan → /tasks → /implement` 那类流水线（见《Spec 驱动开发（Spec-Driven Development）》），本质就是 `spec.md → prompt_plan.md → todo.md` 的产品化。
- **把待办写成仓库里的文件**作为跨会话状态载体，如今已是通用做法——从 `PLAN.md` 协议（《TDD with AI：Kent Beck 的增强编程实践》）到 `fix_plan.md`（《Ralph Wiggum 作为"软件工程师"（Geoffrey Huntley 名篇全文翻译）》）都是同一思想。
- **"一次只问一个问题"的需求澄清式对话**从个人技巧变成工具默认动作（计划模式反问、采访式提示）。
- **绿地与存量分开处理**这个前提被彻底坐实：存量改造的关键是"先把上下文喂进去"，这正是后来上下文工程与仓库索引能力的起点。

**已被工具内置取代（照抄会过时）**

- **用 repomix 手工打包整个仓库再粘贴**：改由智能体的原生仓库检索（glob/grep/语义搜索）、大代码库索引与子智能体隔离调研承担，人不必再当"上下文搬运工"。
- **手写"提示词序列"并逐条粘给模型**：被自主多步执行取代，计划文件本身成了智能体读取并勾选的对象（无人值守形态见《Headless 与 CI 中的 AI 编码》）。
- **文中具体型号与产品名**（ChatGPT 4o / o3 / r1、"claude engineer"、sweep.dev 等）已大量换代或消失，现值以《AI 编程工具全景对比（2026-09 版）》与模型速查为准。
- **"整套流程大概只要 15 分钟"仍然成立，但瓶颈挪了位置**：出计划快到不值钱，值钱的是评审与验证——这正是《AI 代码的安全与质量陷阱：六大陷阱与检测清单》里"大 diff 不可评审"一条记录的账。

**今天该怎么用**

1. 把三段式当**骨架**而非全部：`spec.md` 里必须补上他当年没写的**可验证完成判据**（命令 + 期望输出），否则计划只是更精致的愿望。
2. 存量改造路径值得照抄的是**顺序**（先获取上下文 → 再提改动 → 再验证），不是工具；今天换成 worktree 并行会话 + 仓库检索 + `/code-review`（见《Git in AI 工作流：commit 即存档、worktree 隔离与审查流》）。
3. 他的提示词原文（含 TDD 版与非 TDD 版两份规划提示词）今天仍可直接使用，只需把"给一个代码生成 LLM"改成"给一个带工具的智能体"，并显式禁止它修改测试。
4. 记住作者自己的提醒：**"这套东西现在很好用，可能两周后就不好用了"**——把它当一个时间断面的工作流史来读，而不是当规范。

---

> **来源**：本文主体翻译自 [My LLM codegen workflow atm](https://harper.blog/2025/02/16/my-llm-codegen-workflow-atm/)（作者 Harper Reed，2025-02-16 发布，2025-06 有更新），署名翻译转载（作者公开博客允许署名翻译转载，需署名并附原文链接）。正文全文翻译，未删节知识内容；文中全部提示词为原文照录（保留英文原文以便读者直接使用）。原文内嵌图片未搬运，图意在正文中以文字说明标出；文末作者社交链接与推荐资源从略。抓取于 2026-09-13，开头编者按与文末"2026 年回看"一节为本站编者补充并已标明。
