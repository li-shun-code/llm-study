...
> **以下为本站编者注，非原文内容。** 原文写于 2025 年 2 月，是 AI 代码生成工作流方向被引用最多的文章之一。下面按"已成共识 / 已被工具内置 / 今天不该照抄"三档给出判断，全部是编者观点。

**已成共识（当年这篇属于先行表达）**

- **"先出规格、再出计划、最后才出代码"的三段式**成了主流骨架：计划模式与 `/specify → /plan → /tasks → /implement` 流水线（《Spec 驱动开发（Spec-Driven Development）》）本质就是 `spec.md → prompt_plan.md → todo.md` 的产品化。
- **把待办写成仓库里的文件**作为跨会话状态载体已是通用做法——`PLAN.md`（《TDD with AI：Kent Beck 的增强编程实践》）、`fix_plan.md`（《Ralph Wiggum 作为"软件工程师"》）与它同源。
- **"一次只问一个问题"的需求澄清**从个人技巧变成工具默认动作；**绿地与存量分开处理**这个前提也被彻底坐实。

**已被工具内置（不必再手工做）**

- **用 repomix 手工打包整仓再粘贴**：改由原生仓库检索（glob/grep/语义搜索）、大代码库索引，以及 subagent 在独立上下文里调研、只回摘要承担。
- **手写提示词序列逐条粘给模型**：计划模式批准一次后即可自主多步执行，计划文件由智能体自己读与勾（无人值守见《Headless 与 CI 中的 AI 编码》）。
- **手搓 mise 任务与提示词模板**：`code-review`、`missing-tests` 这类重复动作今天由 Skills 与自定义命令承载；"跑测试、跑 lint"交给 hooks 在每次编辑后自动触发；评审本身也有开箱能力（`/code-review`、云端 PR 评审智能体，见《AI 结对与代码审查》）。
- **文中具体型号与产品名**（4o / o3 / r1、"claude engineer"、sweep.dev）已换代或消失，现值见《AI 编程工具全景对比（2026-09 版）》。

**今天不该照抄**

1. **"整套流程大概只要 15 分钟"仍然成立，但瓶颈挪了位置**：出计划快到不值钱，值钱的是评审与验证——正是《AI 代码的安全与质量陷阱：六大陷阱与检测清单》"大 diff 不可评审"一条记录的账。
2. `spec.md` 里必须补上他当年没写的**可验证完成判据**（命令 + 期望输出），否则计划只是更精致的愿望；逐条怎么落，见《第一次 AI 结对：从零做一个命令行小工具》。
3. 存量改造值得照抄的是**顺序**（先获取上下文 → 再提改动 → 再验证），不是工具；今天换成 worktree 隔离 + 仓库检索（《Git in AI 工作流》）。他的两份规划提示词仍可直接使用，只需把"给一个代码生成 LLM"改成"给一个带工具的智能体"，并显式禁止它修改测试。
4. 记住作者自己的提醒：**"这套东西现在很好用，可能两周后就不好用了"**——当工作流史读，别当规范。
