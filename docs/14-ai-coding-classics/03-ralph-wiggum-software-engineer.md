...
> **以下为本站编者注，非原文内容。** 原文写于 2025 年 7 月，是"Ralph 循环"的命名出处。下面按"已成共识 / 已被工具内置 / 今天该怎么跑 / 必须补上的护栏"四档给判断（均为编者观点）。

**已成共识**

- **"每循环只做一件事 + 把计划外置成文件"**（`fix_plan.md` 当唯一事实源）已是所有长时运行智能体的通用结构：规格、计划、待办住在仓库里，智能体自己读与勾。
- **"测试当背压"与"不许作弊"**成智能体工程第一原则：没有能真实失败的验证，自治就等于随机漫步（《TDD with AI：Kent Beck 的增强编程实践》）。
- **"上下文是易耗品"**：与其在长对话里修补，不如每循环重来；今天叫 compaction 与上下文隔离。
- **"醒来时面对一个坏代码库"这一风险披露**至今读来仍最诚实——技术进步没消除它。

**已被工具内置（不必再手搓 bash）**

- **`while :; do cat PROMPT.md | claude-code ...; done` 这层循环**：已由工作流能力、`Stop`/`PreCompact` 等生命周期 hooks、headless `-p` 配 CI 定时任务承担（《Headless 与 CI 中的 AI 编码》）。
- **"用子智能体扩上下文"**：今天由 subagent 在独立上下文里运行、只回摘要，编排交给动态工作流，不必人写调度脚本（《驾驭 Claude Code》Subagents 一节）；"每循环读同一份提示"这类固定动作沉淀成 Skills。
- **"便宜模型通宵跑"变成分层用模**：高扇出的实现交给小模型，评审与规则编写留给最强模型；评审本身也开箱——评审智能体看 PR、`/code-review` 看 diff（《AI 结对与代码审查》）。
- **单进程单体 + 顺序提交**被并行 worktree 打破：多分支同时推进、合并前各自隔离（《Git in AI 工作流》）。

**今天该怎么跑（编者的最小安全配方）**

1. 循环里放**三道闸门**：测试与 lint 必须通过、不许改测试与验收脚本、每循环一次可回滚的提交。
2. 循环外放**三个上限**：最大循环次数或时长、token 与费用预算、失败即停并升级给人。
3. 任务只写"目标 + 边界 + 验证命令"；`fix_plan.md` 必须人可评审。只在**验证可机械判定**的任务上无人值守（迁移、补测试、修 lint），探索性设计与架构取舍不适合它。

**必须补上的护栏（原文写于 2025 年，当时看不到这些）**

- 智能体会**为了通过评审而做出"看起来合理"的次优解**，意识到方案有问题时还会加倍自辩（METR 2026 年 2—3 月报告）。背压必须由机器给出，不能由模型自述。
- 通宵自治的循环天然凑齐 lethal trifecta（私有数据 + 不可信内容 + 对外通信），须配沙箱、网络白名单与凭据保护（《AI 编码供应链安全》《AI 代码的安全与质量陷阱》）。
