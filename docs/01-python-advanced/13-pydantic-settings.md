...
### 类型不只是标量：DSN、`ImportString` 与「用类型消灭一段 if」

把配置收进类型的真正收益，是**能用现成的类型替掉一段手写解析代码**。官方示例里那几行值得单独看：

```python
from collections.abc import Callable
from typing import Any

from pydantic import BaseModel, ImportString, PostgresDsn, RedisDsn
...
class Settings(BaseSettings):
    ...
    # 字符串会被真正 import 成对象：环境变量里写 "math.cos"，拿到的是函数本体
    special_function: ImportString[Callable[[Any], Any]] = "math.cos"
    # 覆盖方式：export MY_PREFIX_DOMAINS='["foo.com", "bar.com"]'
    domains: set[str] = set()
    # 覆盖方式：export MY_PREFIX_MORE_SETTINGS='{"foo": "x", "apple": 1}'
    more_settings: SubModel = SubModel()

    model_config = SettingsConfigDict(env_prefix="my_prefix_")
```

`RedisDsn`/`PostgresDsn` 会把「是不是合法 DSN」这件事前移到启动期——以前要等第一次建连接才发现 URL 少了一个斜杠。`ImportString` 更适合做**可插拔点的配置**：后端、格式化器、评分函数都可以由一个环境变量指定模块路径，而类型仍然是 `Callable[[Any], Any]`，静态检查不降级。用 LLM 的话说，这就是「把 provider 名字配成一个能解析成具体策略对象」的能力，而不是一堆 `if settings.provider == "..."` 分支。
...
{'environment': 'dev', 'log_level': 'DEBUG', 'http_timeout': 20.0, 'max_connections': 100,
 'max_concurrency': 8, 'request_budget_s': 90.0,
 'cors_origins': ['https://admin.example.com', 'https://localhost:5173'],
 'llm': {'provider': 'deepseek', 'model': 'deepseek-flash', 'api_key': '**********',
         'base_url': 'https://api.deepseek.com', 'temperature': 0.0, 'reasoning_effort': None},
 'legacy_token': None}
...
注意 `'api_key': '**********'`：`mode="json"` 序列化会把 `SecretStr` 遮蔽掉，这份快照可以安全打进日志。
