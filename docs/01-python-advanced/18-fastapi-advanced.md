...
```text
common_parameters（依赖项，只写一次）
├──► /items/    —— 依赖的返回结果注入这个路径操作
└──► /users/    —— 依赖的返回结果注入这个路径操作
```
...
query_extractor（先处理）
└──► query_or_cookie_extractor（再处理，用到上面的返回值）
     └──► /items/（路径操作，接收最终注入结果）
```
...
执行顺序大致如下流程所示。步骤自上而下就是时间轴，每个参与者分别代表交互或执行代码的一部分。

```text
参与者：Client（客户端） · Dep with yield（带 yield 的依赖） · Path Operation（路径操作）
        Exception handler（异常处理器） · Background tasks（后台任务）
范围说明：Client 与 Path Operation 之间的这段流程可能抛出异常，包括 HTTPException。

1.  Client ──► Dep with yield：Start request（开始请求）
2.  Dep with yield → 自身：Run code up to yield（执行到 yield 之前的代码）
    〔分支：yield 之前就抛出异常〕
3.    Dep with yield ──► Exception handler：Raise Exception（抛出异常）
4.    Exception handler ──► Client：HTTP error response（返回 HTTP 错误响应）
5.  Dep with yield ──► Path Operation：Run dependency, e.g. DB session（运行依赖，例如数据库会话）
    〔分支：路径操作抛出异常〕
6.    Path Operation ──► Dep with yield：Raise Exception（例如 HTTPException）
      〔分支：依赖内尝试处理〕
7.      Dep with yield → 自身：Can catch exception, raise a new HTTPException,
        raise other exception（可捕获异常、抛出新的 HTTPException、或抛出其他异常）
8.    Exception handler ──► Client：HTTP error response（返回 HTTP 错误响应）
9.  Path Operation ──► Client：Return response to client（把响应返回给客户端）
    范围说明：到这一步，Client 与 Path Operation 之间「响应已经发送」，此后无法再更改它。
    〔分支：有后台任务〕
10.   Path Operation ──► Background tasks：Send background tasks（发送后台任务）
    〔分支：后台任务代码抛出其他异常〕
11.   Background tasks → 自身：Handle exceptions in the background task code
      （在后台任务的代码中处理异常）
```
...
```text
参与者：Client（客户端） · Dep scope="request"（scope 为 "request" 的依赖）
        Dep scope="function"（scope 为 "function" 的依赖） · Path Operation（路径操作）
步骤自上而下即时间轴：

1.  Client ──► Dep scope="request"：Start request（开始请求）
2.  Dep scope="request" → 自身：Run code up to yield（执行到 yield 之前的代码）
3.  Dep scope="request" ──► Dep scope="function"：Pass dependency（把依赖向下传递）
4.  Dep scope="function" → 自身：Run code up to yield（执行到 yield 之前的代码）
5.  Dep scope="function" ──► Path Operation：Run path operation with dependency
    （带着依赖运行路径操作）
6.  Path Operation ──► Dep scope="function"：Return from path operation（路径操作返回）
7.  Dep scope="function" → 自身：Run code after yield（执行 yield 之后的退出代码）
8.  Dep scope="function" → 自身：✅ Dependency closed（该依赖已关闭）
9.  Dep scope="function" ──► Client：Send response to client（把响应发送给客户端）
10. Client → 自身：Response sent（响应已发送）
11. Dep scope="request" → 自身：Run code after yield（执行 yield 之后的退出代码）
12. Dep scope="request" → 自身：✅ Dependency closed（该依赖已关闭）
```
...
```text
参与者：App（你的应用） · Client（客户端）；步骤自上而下即时间轴

1.  App → 自身：Produce Item 1（生成第 1 项）
2.  App ──► Client：Send Item 1（发送第 1 项）
3.  App → 自身：Produce Item 2（生成第 2 项，客户端此时已在处理第 1 项）
4.  Client → 自身：Process Item 1（处理第 1 项）
5.  App ──► Client：Send Item 2（发送第 2 项）
6.  App → 自身：Produce Item 3（生成第 3 项）
7.  Client → 自身：Process Item 2（处理第 2 项）
8.  App ──► Client：Send Item 3（发送第 3 项）
9.  Client → 自身：Process Item 3（处理第 3 项）
10. App → 自身：Keeps producing...（应用持续产出）
    Client → 自身：Keeps consuming...（客户端持续消费）
```
