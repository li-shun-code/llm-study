...
- 与 Copilot **对话**获得代码帮助。
- 通过**命令行**请求帮助并让智能体在本地改代码：这就是 **GitHub Copilot CLI**（可执行名 `copilot`，与 `gh` 是两个产品；`gh` 另有自己的 Copilot 扩展）。
- 用 **Copilot Spaces** 组织并共享上下文，获得更相关的回答。
- 自动生成**拉取请求描述**。
- 让它**研究、规划、改代码并创建 PR** 供你审查（即 Copilot coding agent，在 GitHub 上以 `@copilot` 认领 issue 产 PR 的形态运行）。

使用场景覆盖 IDE（VS Code、Visual Studio、JetBrains 系、Vim/Neovim 等）、GitHub 网站与 GitHub Mobile、Windows Terminal 聊天、Copilot CLI，以及 **GitHub Copilot App**（面向智能体驱动开发的桌面应用）。访问方式上，个人有 Free/Pro/Pro+/Max 档位，认证学生免费，教师与流行开源项目维护者可免费拿 Pro；组织与企业用 Copilot Business / Enterprise。Copilot 与 GitHub 仓库、issue、PR 流程的深度耦合是其最大差异化。
...
- **模型**：仓库自带的模型清单 `codex-rs/models-manager/models.json` 当前列的是 **GPT-6 Astra**（`gpt-6-astra`，常规上下文 272K、可扩到 872K）、**GPT-5.6 Sol / Terra / Luna**、**GPT-5.5 / GPT-5.4** 以及一个专职审查的 `codex-auto-review`。**早期资料里那批带 `-codex` 后缀的专用名（`gpt-5-codex`、`gpt-5.1-codex-max` 等）在当前清单里已不出现**，CLI 侧命名已与通用前沿线合并；各家通用列表见下一节。
...
三家 CLI 里，Codex CLI 与 Gemini CLI 都开放核心（Claude Code 闭源），而 Codex CLI 又与 agents.md 规范（《AGENTS.md 与 CLAUDE.md：给智能体的项目规范文件》）同出 OpenAI 生态——**想审计智能体在你机器上到底做了什么，这一家最容易**。深度用法见《Codex CLI 深度使用：OpenAI 终端智能体的三层权限、AGENTS.md 与自动化》。
...
**GitHub Copilot（GitHub）**。计费用 **GitHub AI Credits**（**1 credit = $0.01**，例如设 $10 的额外用量预算＝1,000 credits），**代码补全与"下一步编辑建议"不按 credits 计、所有付费档不限次**；额度**不结转**，每月 1 日 00:00:00 UTC 重置。每个付费档的额度分两段：**base**（与订阅价绑定、从不改变）与 **flex allotment**（会随模型经济性调整的赠送段），base 先用完、flex 自动接上，同一额度跨 IDE / 网站 / CLI 共享。

| 档位 | 价格 | AI credits（base + flex = 合计/月） | 模型访问 |
| --- | --- | --- | --- |
| Free | $0 | 有额度（含在自动选模型下） | 仅 Auto model selection；补全 2,000 次/月 |
| Student | $0（认证学生） | 有额度 | 仅 Auto model selection；补全不限次；不含第三方智能体 |
| Pro | $10/月 | 1,000 + 500 = 1,500 | 一批模型 |
| Pro+ | $39/月 | 3,900 + 3,100 = 7,000 | premium 模型 |
| Max | $100/月 | 10,000 + 10,000 = 20,000 | premium 模型优先访问 |
| Business | $19/席·月 | 1,900/人（组织共享池） | premium 模型 |
| Enterprise | $39/席·月 | 3,900/人（组织共享池） | premium 模型优先访问 |

教师与流行开源项目维护者可免费拿 Pro；组织/企业超额部分按 $0.01/credit 计费并计入共享池。**当前可选模型**（官方 plans 页目录，按档位开放）：Claude 一侧有 Haiku 4.5、Sonnet 4.6、**Sonnet 5**、Opus 4.7、Opus 4.8（含 fast mode 预览）、**Opus 5**、Fable 5、**Fable 5.1**；OpenAI 一侧有 GPT-5 mini、GPT-5.3-Codex、GPT-5.4（含 mini / nano）、GPT-5.5、GPT-5.6 Sol / Terra / Luna、GPT-6 Astra；此外还有 Gemini 3.5–3.8 Flash、Grok 4.5 / 4.6、Kimi K2.7 Code / K3、MAI-Code-1.1-Flash。**BYOK 在 Copilot 侧是被支持的**（CLI 与 GitHub Copilot 应用可用自备供应商 key，此时无需 GitHub 登录即可跑模型，代价是拿不到 `/delegate`、GitHub MCP、代码搜索这些依赖 GitHub 的能力）。
...
| --- | --- | --- | --- | --- | --- | --- |
| 形态 | 终端/IDE/桌面/Web | 独立 IDE（VS Code 分支） | 多 IDE 插件 + GitHub 内置 + CLI + 桌面 App + Windows Terminal | VS Code 扩展+CLI+Kanban | 独立 IDE（并入 Devin Desktop） | CLI/IDE 扩展/Web/桌面 |
| 主导交互 | 自然语言智能体循环 | 编辑器内 Agent+补全 | 补全+Chat+云端认领 issue | 批准制智能体循环 | Cascade 规划式智能体 | 智能体循环（本地+云端） |
| 项目规范文件 | CLAUDE.md（兼读 AGENTS.md） | `.cursor/rules`（.mdc）+AGENTS.md | `.github/copilot-instructions.md` + AGENTS.md/CLAUDE.md/GEMINI.md | `.clinerules` | Memories/规则 | AGENTS.md |
| 开源程度 | 闭源（SDK/CLI 分发） | 闭源 | 闭源 | **核心开源（Apache-2.0）** | 闭源 | **CLI 开源（Apache-2.0）** |
| BYOK/自带模型 | 企业网关/Bedrock 等 | 有限 | **CLI 与应用支持 BYOK**（`providers.json`，可脱离 GitHub 登录） | **30+ 供应商/本地模型** | 有限 | API Key 或 ChatGPT 套餐 |
