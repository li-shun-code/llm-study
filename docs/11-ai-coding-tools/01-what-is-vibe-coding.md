...
> 译注：本文所在模块后续篇章——工具对比（《AI 编程工具全景对比（2026-09 版）》）、Claude Code 最佳实践（《Claude Code 工作流与最佳实践》）、Git in AI 工作流（《Git in AI 工作流：commit 即存档、worktree 隔离与审查流》）、代码审查（《AI 结对与代码审查》）、安全陷阱（《AI 代码的安全与质量陷阱》）——正是"vibe engineering / agentic engineering"一侧的各项具体功夫；而《从 0 到 1 用 AI 做产品》会回到"低风险、快迭代"的 vibe coding 一侧，两者结合方为完整图景。
