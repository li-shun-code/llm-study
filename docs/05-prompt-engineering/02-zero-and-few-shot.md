...
本机实测（脚本原样跑一遍，两次运行的输入 token 完全一致）：

```
===== zero =====
[合规] '江苏省|南京市|鼓楼区|中山北路28号 5 号楼 1802'                in=96  out=88
[合规] '四川省|成都市|武侯区|天府三街69号新南中心1栋1204'                in=97  out=178
[合规] '北京市|北京市|朝阳区|建国路88号（收件人临时改到国贸三期B座）'     in=96  out=149
小计：结构合规 3/3，输入 token 289，输出 token 415

===== few =====
[合规] '江苏省|南京市|鼓楼区|中山北路28号5号楼1802'                     in=229 out=98
[合规] '-|成都市|武侯区|天府三街69号新南中心1栋1204'                    in=230 out=152
[不合规] ''                                                            in=229 out=798
小计：结构合规 2/3，输入 token 688，输出 token 1048
```

这组数据比"少样本更好"这个笼统结论有用得多：

1. **表层格式都合规**——四段竖线这种结构，零样本就够了（3/3），示例在这一点上是白花钱；
2. **真正的差别在隐含约定**：直辖市/裸城市名要不要在"省"这一栏填 `-`、详址里的空格要不要吃掉。零样本两次都把"北京"重复填进省市两栏并保留原空格；少样本则从例 1、例 3 学到了口径（`-|成都市|武侯区|…`、空格被合并），输出与下游期望一致；
3. **代价与新增的失败模式都看得见**：输入 token 从 289 涨到 688（约 2.4 倍），输出总量从 415 涨到 1048（2.5 倍）——示例会引导模型"照着例子的详尽程度写"。这一遍少样本组还有一条**空输出**：800 的输出预算被思考 token 吃满（`out=798`），正文一个字没剩。示例不只是多花输入钱，它还会推高整条链路的 token 分布，把边界情况推向超时与截断。
