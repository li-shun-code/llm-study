...
另外要记住：AI 工程本质上是一门经验学科，大语言模型本质上是非确定性的。本文的口径以"指令遵循度更高的通用模型"为主；推理模型（自带思考 token 的那一档）在提示长度、`reasoning_effort` 与"要不要手写步骤"上的取舍不同，另见《推理模型时代的提示原则》。遵循任何指南之外，还应构建有信息量的评估（evals）并频繁迭代，确保提示词的改动确实为你的场景带来收益（详见《提示词的迭代与评估方法》）。
...
5. **输出格式**（Output Format）——与《JSON Mode 与结构化输出（Structured Outputs）》衔接。

设计系统提示词是一项经验工作：改一版、评一轮（《提示词的迭代与评估方法》），永远以评估结果为准。
