...
定量指标之外，还可以给模型的表现打**定性分**：代码质量、任务遵循度等。方法是写一个"评审员"系统提示词（`llm_as_judge.txt`），让另一个模型按统一量表给每份产出打分。评审模型本身有偏差（位置、冗长、自我增强），用之前先按《LLM-as-Judge 的偏差与校准》做一次校准测量，否则分数会把噪声当成进步：
...
这套闭环与《系统提示词设计》的"经验学科"论断互为印证：**提示词工程的每一次改动，都应该由评估结果说话，而不是由感觉说话。**
