...
其他实用指标：`mean_token_accuracy`（SFT 时 token 级准确率，比 loss 更直观）、`grad_norm`（裁剪前梯度范数，长期 >1 说明学习率偏大）、DPO 的 `rewards/accuracies` 与 `rewards/margins`（见 [DPO 与偏好优化](./10-dpo-preference-optimization)）。
