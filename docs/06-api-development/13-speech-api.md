...
> **关于模型名的前缀**：本篇的 `gpt-4o-transcribe`、`gpt-4o-mini-transcribe`、`gpt-4o-mini-tts`、`gpt-4o-audio-preview` 属于**音频产品线的自有命名**——前缀沿用的是这条产品线的名字，不是已经退役的 `gpt-4o` 对话模型，两者不要混为一谈。对话/推理侧的模型档位（`gpt-5.4` 系列、`gpt-6-astra`）见《主流模型生态对比（2026-09）》；音频侧目前仍以 `gpt-4o-*` 与 `tts-1*`、`gpt-realtime-*` 这几族为准，选模型时看的是**能力族**而不是版本号大小。
