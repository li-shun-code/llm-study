...
下一步是把标签升级成**描述句**（descriptions），再用少样本示例让另一个模型把描述润色成商品文案（caption）。原文提示：如果少样本仍达不到目标风格，可以考虑**微调**一个模型来匹配语气与格式（微调见模块 10）。
...
- **视觉 + Function Calling**：让模型看图后决定调用哪个工具（模块 9 的 Computer Use/浏览器 Agent 是其极端形态）。
