...
前面各节是按算法拆开的片段，这里给两个“一个文件跑得完”的例子：一个把检索重排的四步串成微管线，一个把“流式 + 会更新”的榜单写对。都只用标准库。

### 例一：召回 → RRF 融合 → MMR 去重 → token 预算截断

真实检索链路的 Top-K 从来不是“一次 `nlargest` ”，而是**逐级收窄**：每级的候选池大小、是否已排序、要不要去重都不一样。

```python
"""多路召回 -> RRF 融合 -> MMR 多样性重排 -> token 预算截断（仅标准库）"""
import heapq
...
def dense_recall():
    """向量召回：(文档 id, 归一化向量, 与 query 的余弦)。d1 与 d2 是近似重复对"""
    return [("d1", [0.98, 0.20], 0.95), ("d2", [0.97, 0.24], 0.94),
            ("d3", [0.70, 0.71], 0.80), ("d4", [0.10, 1.00], 0.55)]


def rrf_scores(rank_lists, c: int = 60):
    """RRF：文档在某路排第 r 名得 1/(c+r+1) ，多路求和。只要排名，不要可比分数"""
    scores: dict[str, float] = {}
    for ranked in rank_lists:
        for rank, doc_id in enumerate(ranked):
            scores[doc_id] = scores.get(doc_id, 0.0) + 1.0 / (c + rank + 1)
...
def mmr(cands, k: int, lam: float = 0.5):
    """MMR 贪心：每轮取一次 Top-1；打分 = 相关度 - 与已选集合的最大相似度"""
    pool, picked, picked_vecs = list(cands), [], []
    while len(picked) < k and pool:
        best = max(pool, key=lambda d: lam * d[2]
                   - (1 - lam) * max((cos(d[1], p) for p in picked_vecs), default=0.0))
        picked.append(best)
        picked_vecs.append(best[1])
        pool.remove(best)                           # 选完即出池，避免重复选
    return [(d[0], round(d[2], 2)) for d in picked]


def budget_cut(order, sizes, budget: int):
    """按 token 预算截断：像前缀和那样累加，一旦超预算就整条丢掉"""
    used, out = 0, []
    for doc_id in order:
        if used + sizes[doc_id] > budget:
            break
        used += sizes[doc_id]
        out.append(doc_id)
    return out
...
if __name__ == "__main__":
    dense = dense_recall()
    meta = {d[0]: (d[1], d[2]) for d in dense}
    meta["d5"] = ([0.40, 0.90], 0.40)               # 只有稀疏路召回到的文档
    sizes = {"d1": 120, "d2": 130, "d3": 90, "d4": 60, "d5": 45}

    fused = rrf_scores([[d[0] for d in sorted(dense, key=lambda x: -x[2])],
                        ["d4", "d3", "d5", "d1"]])  # 第二路是 BM25 的排名
    print("RRF 前三:", [(d, round(s, 6)) for d, s in
                        sorted(fused.items(), key=lambda kv: (-kv[1], kv[0]))[:3]])
    # 候选池放大：融合后取前 2k ，再交给重排
    cand_ids = [d for d, _ in heapq.nlargest(4, fused.items(), key=lambda kv: kv[1])]
    picked = mmr([(i, *meta[i]) for i in cand_ids], k=3, lam=0.5)
    print("MMR 选出 (id, 相关度):", picked)
    print("预算 250 token 内留下:", budget_cut([p[0] for p in picked], sizes, 250))
```

输出：

```text
RRF 前三: [('d1', 0.032018), ('d4', 0.032018), ('d3', 0.032002)]
MMR 选出 (id, 相关度): [('d1', 0.95), ('d4', 0.55), ('d3', 0.8)]
预算 250 token 内留下: ['d1', 'd4']
```

这段输出把 Top-K 最阴的两个坑演出来了。**第一：`d1` 与 `d4` 的 RRF 分数完全相等** （一路第 1 名 + 另一路第 4 名 = 一路第 4 名 + 另一路第 1 名），此时若不写 `key=lambda kv: (-kv[1], kv[0])` 这样的**显式次级键** ，名次就由 `dict` 插入顺序决定，换台机器、换个分片顺序结果就变。第二：`d2` 相关度 0.94 只比 `d1` 低 0.01 ，却因与 `d1` 几乎同向被 MMR 压掉——**纯 Top-K 按分数取，会把一批近似重复的内容一起塞进 prompt** ，预算花在三份同样的话上。

### 例二：带更新的实时热榜（懒删除堆）

`nlargest` 适合“一批静态数据”，但热搜榜、排行榜的分数是**持续变化**的：`heapq` 不支持“把某个元素的键改大”。标准解法是**字典存当前值 + 堆只追加 + 弹出时校验**（懒删除）：

```python
"""实时热榜：dict 是当前分数唯一真相，堆只追加，弹出时丢弃过期条目"""
import heapq
import itertools
...
class TopKBoard:
    def __init__(self, k: int):
        ...
        self.heap: list[tuple[float, int, str]] = []    # (-分数, 序号, key)：取负当大顶堆
        self._seq = itertools.count()                   # 序号：同分时也可比，不会 TypeError

    def update(self, key: str, delta: float) -> None:
        self.score[key] = self.score.get(key, 0.0) + delta
        heapq.heappush(self.heap, (-self.score[key], next(self._seq), key))

    def top(self) -> list[tuple[str, float]]:
        """取前 k 条：与 dict 里当前分数不符的条目是被覆盖的旧记录，直接丢掉"""
        out, kept = [], []
        while self.heap and len(out) < self.k:
            neg, seq, key = heapq.heappop(self.heap)
            cur = -neg
            if self.score.get(key) != cur:
                continue                                # 懒删除：僵尸条目
            out.append((key, cur))
            kept.append((neg, seq, key))
        self.heap.extend(kept)                          # 有效条目放回，允许继续增量
        heapq.heapify(self.heap)
        return out

    def prune(self) -> tuple[int, int]:
        """僵尸条目堆积时压缩：每个 key 只留最新一条，代价 O(n) 重建"""
        before = len(self.heap)
        self.heap = [(-v, next(self._seq), k) for k, v in self.score.items()]
        heapq.heapify(self.heap)
...
if __name__ == "__main__":
    board = TopKBoard(3)
    for key, delta in [("q1", 5), ("q2", 9), ("q3", 7), ("q1", 4), ("q4", 8), ("q2", -6)]:
        board.update(key, delta)
    print("堆里条目数:", len(board.heap), "实时 Top-3:", board.top())
    # 堆里条目数: 6 实时 Top-3: [('q1', 9.0), ('q4', 8.0), ('q3', 7.0)]
    board.update("q5", 3)
    print("prune 前后条目数:", board.prune())            # (6, 5)：q2 的两条旧记录被清掉
```

**为什么堆会“涨”**：每次更新都追加一条，6 次更新就有 6 条，其中只有 5 个键的当前值有效。这是懒删除的固有代价——**用内存换“不需要任意删除”的实现简单性**。所以要配一个 `prune()` ：当 `len(heap) > 4 * len(score)` 时重建一次，均摊后仍是 O(1) 级别的更新与 O(k log n) 的取榜。另一种解法是“有序结构 + 引用计数”（`sortedcontainers.SortedList` ，第三方 BSD），支持真正的任意删除，代价是引入依赖。
...
9. **并列分数没有次级键**：上文 `d1` 与 `d4` 的 RRF 分完全相等，只写 `-kv[1]` 时名次由插入顺序决定，结果不可复现。**任何对外可见的榜单都要有确定性的 tie-break**（id 升序、时间戳升序皆可）。
10. **先四舍五入再排序**：`round(score, 4)` 会把 0.0320184 与 0.0320179 抹平成同一个值，名次随之颠倒且随精度变化。**排序、比较一律用原始浮点数** ，只在最终展示时格式化。
11. **把动态分数当静态数据**：`nlargest` 是“一次快照”，键的分数会随更新变化时，要么每次全量重算（O(n log k)），要么用上文的“字典 + 懒删除堆”。
12. **MMR 的 λ 与候选池大小一起调**：λ=1 完全退化为按分数取（近似重复全进来），λ 太小又会把强相关文档挤掉；经验区间是 0.5~0.8 ，且**只在 Top-2k ~ Top-4k 的池子里做** ，全库跑 MMR 是 O(k²·n) 的自杀式做法。
