...
一个必须记住的分界：**"给 RAG 加个 rerank" 不是 Agentic RAG**——没有决策分支、没有工具选择，仍是固定管线（《重排序（Rerank）：LLM 打分与 Cross-Encoder 精排》的范围）。Agentic 的最低门槛是：**存在一条会因中间结果而不同的执行路径。**

实现载体上不必迷信某个框架：LangGraph 的状态图、LlamaIndex 的 Workflows、DSPy 的断言，甚至本文这样的一段 while 循环，都能表达同一棵"检索→判定→分支"的决策树。框架解决的是持久化、并行与可视化，不解决"你的 grader 判得准不准"——后者才是回路质量的瓶颈，所以下面先用零依赖实现把回路本身跑通，再考虑换宿主。
...
量"库内无答案占比"的可操作做法：从线上日志抽 200 条真实查询（按天分层抽样，别只抽高峰），每条让标注人回答三档——`本地库能答 / 改写后能答 / 库里根本没有`；同时记录朴素管线在这三类上的首篇相关性。多数团队的第三类落在 5%-15%，且高度集中在特定主题（新规程、外部数据源），这时更划算的往往是**补文档**而不是上 Agent。只有当第三类超过两三成、或"该拒答却硬答"的代价很高（合规、安全操作）时，纠错式回路的投入才开始回本。
