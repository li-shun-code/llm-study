...
把 `alpha` 调成 0.5、0 再各跑一遍，对比 top-K 结果与分数，就能直观评估混合检索相对纯向量检索的收益——当然，严谨的结论仍要回到第 09 篇的评估集上量化。

## 小结

混合检索 = 稠密（语义）+ 稀疏（词法）。它同时拿下两类查询：用户用日常语言描述需求时靠语义检索，用户引用精确的缩写、产品名、错误码时靠词法检索。Anthropic 的上下文检索实验（第 05 篇）也证实：嵌入 + BM25 优于纯嵌入，且两者收益可叠加。下一篇《重排序 Rerank》将展示在混合检索之后如何进一步精排。
