...
领域外没有微调数据时，预训练嵌入往往并不比 BM25 强；而一旦查询是"用户随口说的话"，BM25 又会直接捞空。**混合检索不是"锦上添花"，是把两种失败模式各自堵掉。**
...
**RRF 的 `k0`（默认 60）不是玄学**：它是"第 1 名的优势折价"。`k0` 越小，头名权重越集中（更相信单路的第一名）；越大越平均。经验：两路质量相当用 60；一路明显更可信时调小到 20-30。
...
- 反例：q1（`机柜功率密度太高怎么解决`）向量独有 4 篇里只有一篇相关，融合时反而引入噪声——所以**融合后一定要接重排**（《重排序》那篇），别让 top-20 直接进 prompt。
...
已经用 Postgres 的团队不必再引一个搜索引擎：`tsvector`（配合 `pg_trgm`/中文分词扩展）做词法、`pgvector` 做语义，两条 SQL 各自取 top-N 后在应用层 RRF，或直接用一条 CTE 做窗口排名。**收益是运维面小一个系统**，代价是全文检索的相关性功能（同义词、拼音、纠错）要自己补。详见《PostgreSQL 单库混合检索》一节（模块 7）。
