...
准备向量库（复用第 08 篇的 `encode_pdf` 流程：加载 PDF → 分块 → OpenAI 嵌入 → FAISS）：
...
vectorstore = encode_pdf(path)  # 见《最小 RAG（Simple RAG）系统》
```
...
重排序是显著提升检索信息质量的强力技术：宽召回（初检 k 取大）保证不漏，精排序（LLM 或 Cross-Encoder 重排）保证不滥。配合《文档分块策略：从固定切分到上下文检索（Contextual Retrieval）》 Anthropic 的数据——上下文检索再加重排可把检索失败率降低 67%——它是高级 RAG 实现的必备组件。
