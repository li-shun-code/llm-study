...
> 编者注：分级提示词刻意宽松——"只要包含关键词或语义相关即算相关"，目标是**过滤错误召回**而非苛求精确。这正是生产排障的第一原则：先做便宜的粗筛，再做昂贵的精筛（对应第 11 篇的重排）。
...
若判定为"no"（回答缺乏依据），生产系统应触发重试：重新检索、改写查询（第 13 篇）或降级为"无法回答"——宁可说不知道，也不输出幻觉。
...
输出即为"回答 ← 原文片段"的逐条对照：用户可以点击来源直接核对原文，这正是第 01 篇所说"来源引用建立信任"的落地形态，也呼应第 16 篇 GraphRAG 的证据溯源思想。
...
- **改了分块策略不知道好坏** → 没有评估集就上线，等于盲飞（回到第 09 篇）。
